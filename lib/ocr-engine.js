...
async function buildDatabase(iconPaths) {
  iconDB = {};
  ...
  const loadPromises = iconPaths.map(file => {
    return new Promise(resolve => {
      const img = new Image();
      img.src = `/icons/${file}`;
      img.onload = () => resolve({ file, img });
      img.onerror = () => {
        console.error('Failed to load icon:', file);
        resolve({ file, img: null });
      };
    });
  });
  ...
  for (const { file, img } of loadedImages) {
    if (!img) continue;
    // Nearest neighbor scaling to 32x32
  ...
  console.log(`Loaded ${Object.keys(iconDB).length} icons into memory (with half-scale pyramids).`);
}
...
function scanIcons(canvas, scale = 'auto') {
  ...
  for (let r = 0; r < nativeRows.length - 1; r++) {
    for (let c = 0; c < nativeCols.length - 1; c++) {
      ...
      let searchEndY = Math.floor(expectedCellSize * 28 / 38);
      let searchStartX = Math.floor(expectedCellSize * 2 / 38);
      let searchEndX = Math.floor(expectedCellSize * 19 / 38); // 左半分のみ検索
      ...
      for (let x = searchStartX; x <= searchEndX; x++) {
          ...
          for (let y = searchStartY; y <= searchEndY; y++) {
              ...
              // 灰色の部分が含まれないよう、より厳しい暗さ(Luma<80)で判定
              if (luma < 80) {
                  topY = y;
                  break;
              }
      ...
      for (let x in topBlackYForX) {
          let topY = topBlackYForX[x];
          let sampleY = topY - 1; // Look 1 pixel UP (outside the black border)
          
          debugPoints.push({ x: cx + parseInt(x), y: cy + topY });
          
          if (sampleY >= 0) {
              let pixel = cellMat.ucharPtr(sampleY, parseInt(x));
              let b = pixel[0], g = pixel[1], r = pixel[2];
              sumR += r;
              sumG += g;
              sumB += b;
      ...
      if (sampleCount > 0) {
          maxR = Math.round(sumR / sampleCount);
          maxG = Math.round(sumG / sampleCount);
          maxB = Math.round(sumB / sampleCount);
      } else {
          // 黒枠が見つからなかった場合は空セルの可能性が高いため、COMMON（灰色）にフォールバック
          maxR = 120; maxG = 120; maxB = 120;
      }
      // スケーリング前の処理
    ...
    // If the color is completely gray/white (very low chroma) or pure fallback, it is COMMON/COSMIC
    if (chroma < 40 || maxC < 20) {
        if (maxC > 180) return 'COSMIC';
        return 'COMMON';
    }
    ...
    const grades = [
        ...
        { name: 'ARCANA', r: 150, g: 50, b: 255 },
        { name: 'BEYOND', r: 255, g: 50, b: 150 },
        { name: 'CELESTIAL', r: 0, g: 255, b: 255 },
            ...
            for (let y = 0; y < 16; y++) {
                ...
                for (let x = 0; x < 16; x++) {
                    if (mData[mRow + x] !== 0) {
                        ...
                        sumSq += db*db + dg*dg + dr*dr;
                        sumI2 += iB*iB + iG*iG + iR*iR;
                        sumT2 += tB*tB + tG*tG + tR*tR;
                    }
                }
            }
            
            let denom = Math.sqrt(sumI2 * sumT2);
            let norm = denom > 0 ? sumSq / denom : Infinity;
...
export { scanIcons, loadDatabase, buildDatabase };

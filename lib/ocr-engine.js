...
async function buildDatabase(iconPaths) {
  ...
  // Filter out character images (Hero_*.png) and item box images (Item_9*.png)
  const filteredPaths = iconPaths.filter(file => {
    if (file.startsWith('Hero_')) return false;
    if (file.startsWith('Item_9')) return false; // Item_910011.png, etc.
    return true;
  });

  // Load all images in parallel
  const loadPromises = filteredPaths.map(file => {

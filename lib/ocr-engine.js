  ...
  for (let r = 0; r < nativeRows.length - 1; r++) {
    ...
    return bestName;
      ...
      for (let i = 0; i < allNames.length; i++) {
        ...
        let visiblePixels = 0;
        let visiblePixels16 = 0;
        for (let j = 0; j < 32 * 32; j++) {
            if (mData[j] !== 0) {
                visiblePixels++;
                let px = j % 32;
                let py = Math.floor(j / 32);
                if (px < 16 && py < 16) {
                    visiblePixels16++;
                }
            }
        ...
        for (let dy = 0; dy <= 2; dy++) {
          for (let dx2 = 0; dx2 <= 2; dx2++) {
            ...
            let mse32 = sumSq32 / visiblePixels;
            
            let mse16 = Infinity;
            if (visiblePixels16 > 5) {
                mse16 = sumSq16 / visiblePixels16;
            }
            ...
            if (mse16 === Infinity) {
                avgMse = mse32;
            } else if (mse32 === Infinity) {
                avgMse = mse16;
            } else {
                avgMse = (mse32 + mse16) / 2.0;
            }
      ...
      if (bestGlobalName) {
          // MSEが15000程度の時に0%になるように調整（スクリーンショットのブレを考慮し、判定を大幅に緩める）
          let matchRate = Math.max(0, 100 - (bestGlobalDist / 150.0));
          matchRate = Math.round(matchRate * 10) / 10; // 小数第1位までに丸める

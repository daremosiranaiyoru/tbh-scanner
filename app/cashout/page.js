...
const translations = {
  'en-US': {
    ...
    step2Text: 'After 7 days, log into a global skin trading platform like DMarket using your Steam account. It is a relatively safe platform with millions of users. Select your items and sell them. You can instant-sell to their bots if you\'re in a hurry, or use peer-to-peer trading to get a bit more cash.',
  ...
  'ja-JP': {
    ...
    step2Text: '7日間の待機期間が過ぎたら、世界的なスキン売買プラットフォームである「DMarket」などに自身のSteamアカウントでログインします。数百万人規模のユーザー数がいる比較的安全なプラットフォームになっています。サイト上で該当のアイテムを選択し、売却手続きを行います。すぐに換金したい場合はサイト側の自動買取（BOT）を利用し、少しでも高く売りたい場合はプレイヤー間取引を利用するのが一般的です。',
  ...
  'zh-Hans': {
    ...
    step2Text: '等7天后，用Steam账号直接登录DMarket等全球性的饰品交易平台。这是一个拥有数百万用户的相对安全的平台。选中你的饰品卖掉就行了，想秒出的话可以直接丢给自动收购机器人，想多卖点钱就挂玩家间交易。',
    step3Title: '提现到手（USD）',
  ...
  'zh-Hant': {
    ...
    step2Text: '等7天後，用Steam帳號直接登入DMarket等全球性的飾品交易平台。這是一個擁有數百萬用戶的相對安全的平台。選中你的飾品賣掉就行了，想秒出的話可以直接丟給自動收購機器人，想多賣點錢就掛玩家間交易。',
    step3Title: '提現到手（USD）',
  ...
  'ko-KR': {
    ...
    step2Text: '1주일 지나면 DMarket 같은 세계적인 스킨 거래 플랫폼에 Steam 계정으로 로그인합니다. 수백만 명의 유저가 사용하는 비교적 안전한 플랫폼입니다. 사이트에서 아이템을 선택하고 팔면 됩니다. 급하면 자동 매입 봇한테 바로 넘기고, 조금 더 비싸게 팔려면 유저 간 거래를 쓰는 게 일반적입니다.',
  ...
  'ru-RU': {
    ...
    step2Text: 'Через неделю логинимся через Steam на крупной международной платформе вроде DMarket. Это относительно безопасная платформа, которой пользуются миллионы людей. Выбираем предметы и продаем. Можно слить ботам моментально, или продать реальным игрокам чуть дороже.',
  ...
  'pt-BR': {
    ...
    intro: 'Já se perguntou se você pode transformar os fundos do Steam em dinheiro real? Embora você não possa sacar diretamente do Steam, jogadores veteranos usam sites de skins de terceiros para sacar em dinheiro. Aqui está um rápido resumo de como a comunidade faz isso!',
  ...
  'vi-VN': {
    ...
    step2Text: 'Sau 7 ngày, hãy đăng nhập vào nền tảng giao dịch skin toàn cầu như DMarket bằng tài khoản Steam của bạn. Đây là một nền tảng tương đối an toàn với hàng triệu người dùng. Chọn mặt hàng của bạn và bán chúng. Bạn có thể bán ngay cho bot của họ nếu đang vội hoặc sử dụng giao dịch giữa những người chơi để kiếm thêm một chút tiền.',
